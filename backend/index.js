...
app.use("/images/games", express.static(path.join(__dirname, "public/images/games")));
app.use("/images/setups", express.static(path.join(__dirname, "public/images/setups")));
...
app.post("/users", async(req, res) => {
    try {
        ...
        const result = await pool.query(
            "INSERT INTO users (username, fname, lname, mail, password) VALUES ($1, $2, $3, $4, $5) RETURNING *", [username, fname, lname, mail, password],
        );
...
app.post("/login", async(req, res) => {
    try {
        ...
        const result = await pool.query(
            "SELECT * FROM users WHERE (username = $1 OR mail = $1) AND password = $2", [loginField, password],
        );
...
app.put("/users/:userId/firstname", async(req, res) => {
    try {
        ...
        const result = await pool.query(
            "UPDATE users SET fname = $1 WHERE user_id = $2 RETURNING *", [fname, userId],
        );
...
app.put("/users/:userId/lastname", async(req, res) => {
    try {
        ...
        const result = await pool.query(
            "UPDATE users SET lname = $1 WHERE user_id = $2 RETURNING *", [lname, userId],
        );
...
app.put("/users/:userId/password", async(req, res) => {
    try {
        ...
        const result = await pool.query(
            "UPDATE users SET password = $1 WHERE user_id = $2 RETURNING *", [password, userId],
        );
...
app.put("/users/:userId/username", async(req, res) => {
    try {
        ...
        const checkUsername = await pool.query(
            "SELECT * FROM users WHERE username = $1 AND user_id != $2", [username, userId]
        );

        if (checkUsername.rows.length > 0) {
            return res.status(400).json({ error: "Korisničko ime je već u upotrebi" });
        }
...
app.put("/users/:userId/mail", async(req, res) => {
    try {
        ...
        const checkMail = await pool.query(
            "SELECT * FROM users WHERE mail = $1 AND user_id != $2", [mail, userId]
        );
        ...
        const result = await pool.query(
            "UPDATE users SET mail = $1 WHERE user_id = $2 RETURNING *", [mail, userId]
        );
    ...
    try {
        ...
        const gamesWithDesc = result.rows.map((game) => ({
            ...
            image: game.image_filename ?
                `${BASE_URL}/images/games/${game.image_filename}` : null,
...
app.put("/reservations/:reservationId", async(req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            RETURNING *`, [reservation_date, start_time, duration_hours, reservationId],
        );
...
app.delete("/reservations/:reservationId", async(req, res) => {
    try {
        const { reservationId } = req.params;
        const result = await pool.query(
            "DELETE FROM reservations WHERE reservation_id = $1 RETURNING *", [reservationId],
        );
        if (result.rows.length > 0) {
